...
$sql = "SELECT titulo, descripcion, pu, cantidad, archivo, id_categoria, usuario, fecha FROM articulos";
...
<head>
    ...
    <title>Articulos | PHP Proyecto UTD</title>
    ...
    <section id="content">
       <div class="box">
        ...
        <?php if ($result->num_rows > 0): //si hay resultados ?> 
            <?php while ($row = $result->fetch_assoc()): //un ciclo según los resuldatos obtenidos ?>
                <?php 
                    $categoria = $row['id_categoria']; //variable que guarda el id de la cate
                    $autor = $row['usuario']; //el usuario

                    // obtener el name de la categoriaaa
            ...
            <?php endwhile; ?>
        <?php else: ?>
            <p>No se encontraron artículos, Im sorry</p>
        <?php endif; ?>
       </div>
    </section>

    <footer>
        <p>Creado por PaezOnline &copy; 2024 | Visitado el: 8/11/2024</p>
    </footer>
</body>
</html>
